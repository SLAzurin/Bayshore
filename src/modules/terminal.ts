...
export default class TerminalModule {
  register(app: Application): void {
    ...
    app.post("/method/receive_user_items", async (req, res) => {
      ...
      for (let targetItem of body.targetItemIds) {
        ...
        if (item) {
          ...
          console.log(
            "Warning: Item " + targetItem + " not found. Item not added."
          );
        }
      }
    ...
    app.post("/method/save_scratch_sheet", async (req, res) => {
      ...
      try {
        // Attempt to update scratch sheet
        ...
        error // Failed to update scratch sheet
      ) {
    ...
    app.post("/method/load_ghost_competition_ranking", async (req, res) => {
      // Get the information from the request
      let body = wm.wm.protobuf.LoadGhostCompetitionRankingRequest.decode(
        req.body
      );
        ...
        if (
          ...
          if (ocmParticipant) {
            ...
            for (let i = 0; i < ocmParticipant.length; i++) {
              ...
              if (
                ocmGhostrecord?.playedShopName !== null &&
                ocmGhostrecord?.playedShopName !== undefined
              ) {
              ...
              if (ocmParticipant[i].carId === body.carId && ranking === 0) {
                // User car setting
                ownRecords =
                  wm.wm.protobuf.LoadGhostCompetitionRankingResponse.Entry.create(
                    {
                      rank: i + 1,
      ...
      // Encode the response
      let message =
        wm.wm.protobuf.LoadGhostCompetitionRankingResponse.encode(msg);
