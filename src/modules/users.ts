...
export default class UserModule {
  register(app: Application): void {
    ...
    app.post("/method/load_user", async (req, res) => {
      ...
      if (
        body.cardChipId.match(/7F5C9744F11111114326.*/) ||
        body.cardChipId.match(/0000000000.*/)
      ) {
        body.cardChipId = "";
        body.accessCode = "";
      }
      ...
      if (!user) {
        ...
        if (newCardsBanned === 0) {
          ...
          let ftTicketGrant =
            Config.getConfig().gameOptions.grantFullTuneTicketToNewUsers;

          if (ftTicketGrant > 0) {
            console.log(
              `Granting Full-Tune Ticket x${ftTicketGrant} to new user...`
            );
      ...
      if (user.carOrder.length > 0) {
        ...
        user.cars = user.cars.sort(function (a, b) {
          ...
          if (user) {
            ...
            let compare: number =
              user?.carOrder.indexOf(a!.carId) -
              user?.carOrder.indexOf(b!.carId);

            // Return the comparison
            return compare;
          } // Car IDs not present in car order list
          else {
      ...
      if (user.cars.length > 0) {
        ...
        if (
          user.cars[0]?.windowStickerString !== null &&
        ...
        if (
          user.cars[0].lastPlayedPlaceId === null ||
          user.cars[0].lastPlayedPlaceId === undefined
        ) {
      ...
      let msg = {
        ...
        competitionUserState:
          wm.wm.protobuf.GhostCompetitionParticipantState
            .COMPETITION_NOT_PARTICIPATED,
      ...
      for (let i = 0; i < msg.cars.length; i++) {
        ...
        if (user.cars.length > 0 && ocmEventDate) {
          ...
          if (
            ocmEventDate!.competitionStartAt < date &&
            ocmEventDate!.competitionCloseAt > date
          ) {
            ...
            if (checkParticipation) {
              ...
              if (ocmTallyRecord) {
                ...
                carStates[i].competitionState =
                  wm.wm.protobuf.GhostCompetitionParticipantState.COMPETITION_QUALIFIED;
              }
            } else {
              carStates[i].eventJoined = false;
              carStates[i].competitionState =
                wm.wm.protobuf.GhostCompetitionParticipantState.COMPETITION_NOT_PARTICIPATED;
            }
          }
          // Current date is OCM qualifying day
          else if (
            ocmEventDate!.qualifyingPeriodStartAt < date &&
            ocmEventDate!.qualifyingPeriodCloseAt > date
          ) {
            // Check ocm play record
            let checkParticipation = await prisma.oCMPlayRecord.findFirst({
              where: {
            ...
            if (checkParticipation) {
              ...
              if (ocmRecord) {
                carStates[i].eventJoined = true;
                carStates[i].competitionState =
                  wm.wm.protobuf.GhostCompetitionParticipantState.COMPETITION_PARTICIPATED;
              }
            } else {
              carStates[i].eventJoined = false;
              carStates[i].competitionState =
          ...
          // Current date is OCM ended
            ...
            ocmEventDate!.competitionCloseAt < date &&
            ocmEventDate!.competitionEndAt > date
          ) {
            // Check ocm play record
            let checkParticipation = await prisma.oCMPlayRecord.findFirst({
              where: {
        ...
        let checkRegisteredGhost =
          await prisma.ghostRegisteredFromTerminal.findFirst({
            where: {
              carId: user.cars[i].carId,
            },
          });
    ...
    app.post("/method/load_drive_information", async (req, res) => {
      ...
      let noticeWindows = notice.map(
        (a) => wm.wm.protobuf.NoticeEntry.NOTICE_UNUSED_1
      );
