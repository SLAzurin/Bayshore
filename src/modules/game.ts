...
export default class GameModule {
  register(app: Application): void {
    ...
    app.post("/method/save_game_result", async (req, res) => {
      ...
      let timestamps = 0;
      if (
        body.car?.lastPlayedAt !== undefined &&
        body.car?.lastPlayedAt !== null
      ) {
      ...
      await prisma.car.update({
        ...
        data: {
          ...
          stampSheetCount: common.sanitizeInputNotZero(
            body.rgResult?.stampSheetCount
          ),
        },
      });
      ...
      if (user) {
        ...
        body.confirmedTutorials.forEach((idx) => (storedTutorials[idx] = true));
    ...
    app.post("/method/load_game_history", async (req, res) => {
      ...
      for (let record of records) {
        ...
        ta_records.push(
          wm.wm.protobuf.LoadGameHistoryResponse.TimeAttackRecord.create({
            course: record.course,
            time: record.time,
      ...
      for (let i = 0; i < ghostHistoryData.length; i++) {
        // User car setting
        let carSetings =
          wm.wm.protobuf.LoadGameHistoryResponse.GhostBattleRecord.GhostCarSetting.create(
            {
        ...
        ghostOpponentCar!.tuneHandling =
          ghostHistoryData![i].opponent1TuneHandling!;

        // Get the Data
        let ghostOpponent =
          wm.wm.protobuf.LoadGameHistoryResponse.GhostBattleRecord.GhostBattleRecordCar.create(
            {
              car: {
                ...ghostOpponentCar!,
              },
        ...
        // ----Ghost Opponent 2----
        if (
          ghostHistoryData[i]?.opponent2CarId !== null &&
          ghostHistoryData[i]?.opponent2CarId !== undefined
        ) {
          ...
          // Get Opponent 2 tune
          ghostOpponentCar2!.tunePower =
            ghostHistoryData![i].opponent2TunePower!;
          ghostOpponentCar2!.tuneHandling =
            ghostHistoryData![i].opponent2TuneHandling!;
        ...
        // ----Ghost Opponent 3----
        if (
          ghostHistoryData[i]?.opponent3CarId !== null &&
          ghostHistoryData[i]?.opponent3CarId !== undefined
        ) {
          ...
          // Get Opponent 3 tune
          ghostOpponentCar3!.tunePower =
            ghostHistoryData![i].opponent3TunePower!;
          ghostOpponentCar3!.tuneHandling =
            ghostHistoryData![i].opponent3TuneHandling!;
        ...
        list_ghostHistoryData.push(
          wm.wm.protobuf.LoadGameHistoryResponse.GhostBattleRecord.create({
