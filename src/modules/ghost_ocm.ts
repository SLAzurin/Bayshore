...
export default class GhostModule {
  register(app: Application): void {
    ...
    app.post("/method/load_ghost_competition_info", async (req, res) => {
      ...
      let body = wm.wm.protobuf.LoadGhostCompetitionInfoRequest.decode(
        req.body
      );
      ...
      if (ocmEventDate) {
        ...
        if (ocmPeriodCount === 0) {
          ...
          while (
            competitionPeriodStartTimestamp < ocmEventDate.competitionCloseAt
          ) {
            // Count period closing timestamp
            competitionPeriodEndTimeStamp =
              competitionPeriodStartTimestamp + ocmEventDate.lengthOfPeriod;
            ...
            period++;
            competitionPeriodStartTimestamp =
              competitionPeriodEndTimeStamp + ocmEventDate.lengthOfInterval;
          }

          // Check the gap between quali close and main draw start timestamp
          let checkQualiMainGap =
            ocmEventDate.competitionStartAt -
            ocmEventDate.qualifyingPeriodCloseAt;
        ...
        if (
          ...
          if (OCMCurrentPeriod) {
            ...
            // If not yet tallying
            if (OCMTallyCount === 0) {
              await ghost_ocm.ocmTallying(
                body,
                OCMCurrentPeriod.periodId,
                false
              );
            ...
            let ocmCompetitionDay = await ghost_ocm.ocmCompetitionDay(
              body,
              OCMCurrentPeriod!.competitionId,
              OCMCurrentPeriod!.periodId
            );
        ...
        // Current date is OCM qualifying day
          ...
          // Get Competition Day Event Data for the car
          let ocmCompetitionDay = await ghost_ocm.ocmQualifyingDay(
            body,
        ...
        // OCM has ended
          ...
          if (OCMCurrentPeriod) {
            ...
            if (checkOneParticipant) {
              ...
              if (checkNameplate === 0) {
                await ghost_ocm.ocmGiveNamePlateReward(
                  ocmEventDate.competitionId
                );
              }
      ...
      // Current date is OCM qualifying day
        ...
        if (checkGhostTrail) {
          ...
          if (ocmEventDate) {
            ...
            competitionSchedule =
              wm.wm.protobuf.GhostCompetitionSchedule.create({
