...
export default class ResourceModule {
  register(app: Application): void {
      ...
      for (let i = 0; i < 25; i++) {
        ...
        if (ta_time.length > 0) {
          ...
          for (let j = 0; j < ta_time.length; j++) {
            ...
            list_ta.push(
              wmsrv.wm.protobuf.Ranking.Entry.create({
                carId: ta_time[j].carId,
                rank: car_ta!.level,
          ...
          lists.push(
            new wmsrv.wm.protobuf.Ranking.List({
        ...
        for (let i = 0; i < 19; i++) {
          ...
          if (car_crown[counter].area === i) {
            ...
            if (
              ...
              // Acquired crown timestamp - 1 day
              car_crown[counter].playedAt =
                car_crown[counter].playedAt - 172800;
            }
            ...
            // GID_RUNAREA_C1 - GID_RUNAREA_TURNPIKE
              ...
              list_crown.push(
                wmsrv.wm.protobuf.Crown.create({
                  carId: car_crown[counter].carId,
                  area: car_crown[counter].area,
                  unlockAt: car_crown[counter].playedAt,
                  car: car!,
    ...
    app.get("/static/:filename", async function (req, res) {
      ...
      res.sendFile(path.resolve(paths!.filePath, req.params.filename), {
        cacheControl: false,
      });
    });
    ...
    app.get("/resource/file_list", async (req, res) => {
      ...
      for (let i = 0; i < fileList.length; i++) {
        files.push(
          wm.wm.protobuf.FileList.FileInfo.create({
            fileId: fileList[i].fileId,
            fileType: fileList[i].fileType,
            fileSize: fileList[i].fileSize,
            url:
              "https://" +
              Config.getConfig().serverIp +
              ":9002/static/" +
              fileList[i].urlFileName,
