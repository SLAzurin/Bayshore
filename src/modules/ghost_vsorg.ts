...
export default class GhostModule {
  register(app: Application): void {
    ...
    app.post(
      "/method/load_ghost_expedition_target_by_path",
      async (req, res) => {
        // Get the request body for the load stamp target request
        let body = wm.wm.protobuf.LoadGhostExpeditionTargetByPathRequest.decode(
          req.body
        );
        ...
        for (let j = 0; j < 19; j++) {
          ...
          areaExpedition.push(
            wm.wm.protobuf.LoadGhostExpeditionTargetByPathResponse.AreaStats.create(
              {
                area: areaVal,
                path: pathVal,
                wantedInfo: null,
        ...
        // Encode the response
        let message =
          wm.wm.protobuf.LoadGhostExpeditionTargetByPathResponse.encode(msg);
    ...
    app.post("/method/load_ghost_expedition_targets", async (req, res) => {
      // Get the request body for the load stamp target request
      let body = wm.wm.protobuf.LoadGhostExpeditionTargetsRequest.decode(
        req.body
      );
      ...
      let path = 0;
      if (body.path) {
        if (body.path >= 0 && body.path <= 9) {
          // GID_PATH_C1
          area = Number(0);
          ramp = 0;
        } else if (body.path >= 10 && body.path <= 15) {
          // GID_PATH_N9
          area = Number(1);
          ramp = 4;
        } else if (body.path >= 16 && body.path <= 17) {
          // GID_PATH_WTEAST
          area = Number(2);
          ramp = 6;
        } else if (body.path >= 18 && body.path <= 19) {
          // GID_PATH_WT_UP_DOWN
          area = Number(3);
          ramp = 8;
        } else if (body.path >= 20 && body.path <= 26) {
          // GID_PATH_WG
          area = Number(4);
          ramp = 10;
        } else if (body.path >= 27 && body.path <= 33) {
          // GID_PATH_KG
          area = Number(5);
          ramp = 14;
        } else if (body.path >= 34 && body.path <= 37) {
          // GID_PATH_YS
          area = Number(6);
          ramp = 18;
        } else if (body.path >= 38 && body.path <= 48) {
          // GID_PATH_KG_SHINYAMASHITA_MINATOMIRAI
          area = Number(7);
          ramp = 21;
        } else if (body.path === 49) {
          // GID_PATH_NGR
          area = Number(8);
          ramp = 25;
        } else if (body.path >= 50 && body.path <= 53) {
          // GID_PATH_OS
          area = Number(9);
          ramp = 26;
        } else if (body.path >= 54 && body.path <= 55) {
          // GID_PATH_KB
          area = Number(10);
          ramp = 27;
        } else if (body.path >= 58 && body.path <= 61) {
          // GID_PATH_FK
          area = Number(11);
          ramp = 29;
        } else if (body.path >= 62 && body.path <= 63) {
          // GID_PATH_HK
          area = Number(12);
          ramp = 33;
        } else if (body.path >= 64 && body.path <= 65) {
          // GID_PATH_TP
          area = Number(13);
          ramp = 35;
        } else if (body.path >= 56 && body.path <= 57) {
          // GID_PATH_HS
          area = Number(18);
          ramp = 37;
        }
      ...
      for (let i = 0; i < localScores.length; i++) {
        ...
        if (localScores[i].carId !== body.carId) {
          ...
          recentWinners.push(
            wm.wm.protobuf.CarEntry.create({
              name: car!.name,
              level: car!.level,
              title: car!.title,
              model: car!.model,
      ...
      for (let i = 0; i < wantedCarList.length; i++) {
        ...
        if (wantedCar) {
          ...
          lists_wanted.push(
            wm.wm.protobuf.WantedCar.create({
              ghost: ghostcar,
      ...
      // Encode the response
      let message =
        wm.wm.protobuf.LoadGhostExpeditionTargetsResponse.encode(msg);
    ...
    app.post("/method/load_ghost_expedition_result", async (req, res) => {
      // Get the request body for the load stamp target request
      let body = wmsrv.wm.protobuf.LoadGhostExpeditionResultRequest.decode(
        req.body
      );
      ...
      for (let i = 0; i < localScores.length; i++) {
        ...
        if (car) {
          ghostExpeditionRankings.push(
            wm.wm.protobuf.GhostExpeditionRankingEntry.create({
              rank: i + 1,
              score: localScores[i].score,
              car: car!,
            })
      ...
      // Encode the response
      let message =
        wmsrv.wm.protobuf.LoadGhostExpeditionResultResponse.encode(msg);
